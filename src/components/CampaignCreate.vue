<template>
  <span>
    ...
    <b-list-group v-else>
      <div class="campaignGrid mb-3" v-for="campaign in campaigns" :key="campaign.id">
        <b-list-group-item
          button
          @click="startGame(campaign.id, null)"
        >{{ `Owned Campaign: ${campaign.data.name}` }}</b-list-group-item>
        <b-button
        ...
        >X</b-button>
        <small>{{ `Campaign ID: ${campaign.id} | Campaign Code: ${campaign.data.code}` }}</small>
      </div>
      <div
      ...
      .onSnapshot(res => {
        ...
        res.forEach(character => {
          this.characters.push({
            ...
            value: { id: character.id, name: character.data().characterName }
          });
        });
      });
